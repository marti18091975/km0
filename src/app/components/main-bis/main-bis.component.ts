...
export class MainBisComponent implements OnInit ,AfterViewInit,OnDestroy {
  ...
  public productExistSubscription:Subscription;
  ...
  generateIcons(products){
    for(let n in products){
      const point=new Point(olProj.fromLonLat([0, 0]));
      point.setCoordinates(olProj.fromLonLat([products[n].producerLongitude, products[n].producerLatitude]));
      ...
      marker.setId(products[n].producerEmail);
